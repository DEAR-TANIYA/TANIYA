...
async def auto_filter(client, msg, spoll=False):
    ...
    if offset != "":
        ...
        req = message.from_user.id if message.from_user else 0
        btn.append(
            [InlineKeyboardButton(text=f"⎙ 𝗣𝗔𝗚𝗘 1/{math.ceil(int(total_results) / 6)}", callback_data="pages"),
             InlineKeyboardButton(text="𝗡𝗘𝗫𝗧 ⌦", callback_data=f"next_{req}_{key}_{offset}")]
        )
    else:
        btn.append(
            [InlineKeyboardButton(text="⎙ 𝗣𝗔𝗚𝗘", callback_data="pages")]
        )
...
async def pm_AutoFilter(client, msg, pmspoll=False):
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text=f"⎙ 𝗣𝗔𝗚𝗘 1/{math.ceil(int(total_results) / 6)}", callback_data="pages"),
            InlineKeyboardButton(text="𝗡𝗘𝗫𝗧 ⌦", callback_data=f"pmnext_{req}_{key}_{offset}")]
        )
    else:
        btn.append(
            [InlineKeyboardButton(text="⎙ 𝗣𝗔𝗚𝗘", callback_data="pages")]
